...
public class IWBundleStarter implements IWBundleStartable {
	...
	private static TimerEntry oauthTokensCacheTimer = null;
	...
	public void start(IWBundle starterBundle) {
		...
		startOAuthTokensCacheDaemon(starterBundle);
		getOAuth2DAO().doCacheAccessTokens();
	...
	private void startOAuthTokensCacheDaemon(IWBundle iwb) {
		IWMainApplication iwma = iwb.getApplication();
		IWMainApplicationSettings settings = iwma.getSettings();
		String propName = "oauth.handle_tokens_cache";
		if (!settings.getBoolean(propName, true)) {
			return;
		}
		...
		if (oauthTokensCacheTimer == null) {
			try {
				oauthTokensCacheTimer = tManager.addTimer(30, 3, -1, -1, -1, -1, new TimerListener() {
					...
					public void handleTimer(TimerEntry entry) {
						try {
							...
							LOGGER.info("Starting to handle OAuth 2 tokens cache");
							getOAuth2DAO().doCacheAccessTokens();
							LOGGER.info("Finished handling OAuth 2 tokens cache");
						} catch (Exception e) {
							LOGGER.log(Level.WARNING, "Error handling OAuth 2 tokens cache", e);
						}
					}

				});
				if (oauthTokensCacheTimer != null) {
					oauthTokensCacheTimer.setRepetitive(true);
				}
			} catch (Exception e) {
				oauthTokensCacheTimer = null;
				LOGGER.log(Level.WARNING, "Error starting OAuth 2 tokens cache", e);
			}
		}
	}
